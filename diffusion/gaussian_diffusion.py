...
class GaussianDiffusion(nn.Module):
    def __init__(self, config, mean_type, device, history_num_per_term=10, beta_fixed=False):
        super(GaussianDiffusion, self).__init__()
        self.config = config
        self.device = device
        self.hidden_size = config['latent_dim']
        self.steps = config['steps']

        in_dims = eval(config['mlp_dims'])[::-1]
        ### Build MLP ###
        latent_size = in_dims[-1]
        mlp_out_dims = eval(config['mlp_dims']) + [latent_size]
        mlp_in_dims = mlp_out_dims[::-1]
        self.model = DNN(mlp_in_dims, mlp_out_dims, config['time_emb_size'], config['guidance_emb_size'], time_type="cat", norm=config['norm'],
                    act_func=config['mlp_act_func']).to(self.device)

        if config['optimizer2'] == 'Adagrad':
            self.server_opt = torch.optim.Adagrad(self.model.parameters(), lr=config['lr_server'], initial_accumulator_value=1e-8, weight_decay=config['wd2'])
        elif config['optimizer2'] == 'Adam':
            self.server_opt = torch.optim.Adam(self.model.parameters(), lr=config['lr_server'], weight_decay=config['wd2'])
        elif config['optimizer2'] == 'AdamW':
            self.server_opt = torch.optim.AdamW(self.model.parameters(), lr=config['lr_server'], weight_decay=config['wd2'])
        elif config['optimizer2'] == 'SGD':
            self.server_opt = torch.optim.SGD(self.model.parameters(), lr=config['lr_server'], weight_decay=config['wd2'])
        elif config['optimizer2'] == 'Momentum':
            self.server_opt = torch.optim.SGD(self.model.parameters(), lr=config['lr_server'], momentum=0.95, weight_decay=config['wd2'])
        ...
        self.Lt_history = th.zeros(self.steps, history_num_per_term, dtype=th.float64).to(device)
        self.Lt_count = th.zeros(self.steps, dtype=int).to(device)
    ...
    def training_losses(self, x_start, guidance, reweight=False):
        self.server_opt.zero_grad()

        batch_size, device = x_start.size(0), x_start.device
        ts, pt = self.sample_timesteps(batch_size, device, self.config['time_sample_strategy'])
        ...
        terms = {}
        model_output = self.model(x_t, ts, guidance)
        target = {
        ...
        loss = terms["loss"].mean()  # loss from diffusion
        latent_embedding = terms["pred_xstart"]
    ...
    def p_mean_variance(self, x, t, guidance):
        """
        ...
        B, C = x.shape[:2]
        assert t.shape == (B, )
        model_output = self.model(x, t, guidance)
    ...
    def predict(self, dataloader):
        ...
        for batch in dataloader:
            target, guidance = batch['target'], batch['guidance']
            x_t = torch.randn(guidance.shape[0], self.hidden_size).to(self.device)
            times = list(range(0, self.steps))[::-1]
            for i in times:
                t = th.tensor([i] * x_t.shape[0]).to(x_t.device)
                guidance = guidance.to(x_t.device)
                model_output = self.p_mean_variance(x_t, t, guidance)
                if self.config['sampling_noise']:
                    noise = th.randn_like(x_t)
                    nonzero_mask = (
                        (t != 0).float().view(-1, *([1] * (len(x_t.shape) - 1)))
                    )  # no noise when t == 0
                    x_t = model_output["mean"] + nonzero_mask * th.exp(0.5 * model_output["log_variance"]) * noise
                else:
